#!/usr/bin/env python3
"""Scan SQL dump string literals for likely text corruption."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Scan INSERT payloads for likely text corruption after SQL decoding."
    )
    ...
    parser.add_argument(
        "--columns",
        default="",
        help="Comma-separated column names to limit scanning; defaults to all string literals",
    )
    parser.add_argument(
        "--limit",
        type=int,
...
def _analyze_text(decoded: str) -> list[tuple[str, str, str, str]]:
    findings: list[tuple[str, str, str, str]] = []
    if "\ufffd" in decoded:
        findings.append((
            "high",
            "replacement_character",
            "Decoded text contains Unicode replacement characters.",
            "This usually means the source dump had undecodable bytes. Inspect the original SQLite export for encoding issues.",
        ))
    if "\x00" in decoded:
        findings.append((
            "high",
            "nul_byte",
            "Decoded text contains NUL bytes.",
            "NUL bytes are risky for MySQL text import. Inspect and clean this source value explicitly.",
        ))
    bad_controls = sorted(
        {ord(ch) for ch in decoded if ord(ch) < 32 and ch not in "\n\r\t"}
    )
    if bad_controls:
        findings.append((
            "medium",
            "control_characters",
            f"Decoded text contains control characters: {', '.join(hex(code) for code in bad_controls)}.",
            "Inspect this value before import; hidden control bytes can cause truncation or application-side parse issues.",
        ))
    for marker in _MOJIBAKE_MARKERS:
        if marker in decoded:
            findings.append((
                "medium",
                "possible_mojibake",
                f"Decoded text contains suspicious mojibake marker {marker!r}.",
                "Compare this value with a known-good SQLite/Grafana source before import.",
            ))
            break
    return findings
...
def _scan_statement(
    ...
    search_offset = match.start("values")
    for row in rows:
        row_values = row[1:-1] if row.startswith("(") and row.endswith(")") else row
        ...
        for column, token in zip(columns, tokens):
            ...
            if decoded is None:
                continue
            for severity, code, detail, recommendation in _analyze_text(decoded):
                findings.append(
                    TextFinding(
                        severity=severity,
                        code=code,
    ...
    with open(dump_file, "r", encoding="utf-8", errors="replace") as fp:
        for statement, start_line, end_line in statement_splitter(fp):
            ...
            if table_filter and short_name not in table_filter:
                continue
            findings.extend(_scan_statement(statement, start_line, end_line, column_filter))
    return findings


def _render_text(findings: list[TextFinding], limit: int) -> str:
    ...
    if not findings:
        lines.append("No likely text corruption markers found in the checked string literals.")
    return "\n".join(lines)
...
def main(argv: Iterable[str]) -> int:
    ...
    findings = scan_dump(args.dump_file, table_filter, column_filter)
    if args.json:
